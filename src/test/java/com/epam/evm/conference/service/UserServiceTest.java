...
//        UserPersistentDao dao = Mockito.mock(UserPersistentDao.class);
//        Mockito.when(helper.createUserPersistentDao()).thenReturn(dao);
...
//        Mockito.when(validator.isValidLength(anyString())).thenReturn(true);
...
//        Mockito.when(dao.findUserByLoginAndPassword("admin", "pass")).thenReturn(Optional.of(new User(null, "USER", "user", "pass")));
//        FieldUtils validator = Mockito.mock(FieldUtils.class);
//        Mockito.when(validator.isValidLength(anyString())).thenReturn(false);
//        UserService service = new UserService(factory, validator);
//        //when
