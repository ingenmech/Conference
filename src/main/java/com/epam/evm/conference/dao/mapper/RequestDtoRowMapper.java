...
public class RequestDtoRowMapper implements RowMapper<RequestDto> {
    ...
    @Override
    public RequestDto map(ResultSet resultSet) throws DaoException {
        try {
            ...
            return new RequestDto(id, sectionId, userId, topic, status, conferenceName,
                    sectionName, userLogin, sectionStatus, dateTime);
        } catch (SQLException e) {
            throw new DaoException("Error RequestDtoRowMapper", e);
        }
    }
}
